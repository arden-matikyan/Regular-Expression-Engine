...
let rec regexp_to_nfa (regexp: regexp_t) : (int, char) nfa_t =
    ...
    | Empty_String -> 
      let s = fresh() in
      {sigma = []; qs = [s]; q0 = s; fs = [s]; delta = [];}
    | Char char -> 
      let s0 = fresh() in
      let s1 = fresh() in
      {sigma = [char]; qs = [s0; s1]; q0 = s0; fs = [s1]; delta = [(s0, Some char, s1)];}
    | Union (regexp1, regexp2) ->
      let s0 = fresh() in
      let s1 = fresh() in
      let m1 = regexp_to_nfa regexp1 in 
      let m2 = regexp_to_nfa regexp2 in 
      {sigma = union m1.sigma m2.sigma; qs = union (union m1.qs m2.qs) [s0; s1]; q0 = s0; fs = [s1]; delta = union (union m1.delta m2.delta) [(s0, None, m1.q0);(s0, None, m2.q0);(List.hd m1.fs, None, s1);(List.hd m2.fs, None, s1)];}
    | Concat (regexp1, regexp2) -> 
      let m1 = regexp_to_nfa regexp1 in 
      let m2 = regexp_to_nfa regexp2 in 
      {sigma = union m1.sigma m2.sigma; qs = union m1.qs m2.qs; q0 = m1.q0; fs = m2.fs; delta = union (union m1.delta m2.delta) [(List.hd m1.fs, None, m2.q0)] }
    | Star regexp1 -> 
      let m1 = regexp_to_nfa regexp1 in 
      let s0 = fresh() in 
      let s1 = fresh() in  
      {sigma = m1.sigma; qs = union m1.qs [s0; s1]; q0 = s0; fs = [s1]; delta = union m1.delta [(List.hd m1.fs, None, s1);(s0, None, s1);(s0, None, m1.q0);(s1, None, s0)];} 
